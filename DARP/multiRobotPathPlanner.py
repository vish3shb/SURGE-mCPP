...
config_path = os.path.join(parent_dir, 'config.json5')
#config_path = '/home/samshad/Documents/multi_uav/CA_DARP/config.json5'
with open(config_path, 'r') as config_file:
    config = json5.load(config_file)
# Global variable for node distance
NODE_DISTANCE = config.get('Path_step_size')
...
class MultiRobotPathPlanner(DARP):
    def __init__(self, nx, ny, notEqualPortions, initial_positions, portions,
        ...
        if not self.DARP_success:
            ...
            # Save paths
            self.path_files = self.save_paths_txt(self.best_case.paths, base_dir)

            
    def save_paths_txt(self, paths, directory):
        """
        Save each path to a separate text file and return the file paths.
        :param paths: List of paths for each robot
        :param directory: Directory to save the paths
        ...
        for i, path in enumerate(paths):
            file_path = os.path.join(directory, f"uav_{i+1}_path.txt")
            with open(file_path, 'w') as file:
                for point in path:
                    # Adjust coordinates based on the global NODE_DISTANCE
                    x_coord = point[1] * NODE_DISTANCE + (NODE_DISTANCE / 2)
                    y_coord = point[0] * NODE_DISTANCE + (NODE_DISTANCE / 2)
                    file.write(f"{x_coord:.2f}\t{y_coord:.2f}\n")
            print(f"Path for robot {i+1} saved to {file_path}")
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        ...
        default=15,
        type=int,
        help='Distance between adjacent nodes (default: 15)')
    args = argparser.parse_args()

    NODE_DISTANCE = args.dist
